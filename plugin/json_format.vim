...
function! json_format#parse(mode)
    ...
    silent execute ":"dictCmd[a:mode]"!python -c 'import json,sys;reload(sys);sys.setdefaultencoding(\"utf-8\");js = json.loads(sys.stdin.read().strip());print json.dumps(js, sort_keys=False, indent=2, ensure_ascii=False);'"
endfunction
